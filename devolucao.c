...
typedef struct  Livro 
{
    ...
    int emprestado;     //Indicador se o Livro está emprestado (0 = não / 1 = sim)
    char emprestadoPara[100];   //Nome da pessoa para quem o livro foi emprestado
...
Livro * criar (int id, const char *titulo, const char *autor, int ano, const char *genero) {
    ...
    strcpy(novo->emprestadoPara, "");   //Inicializa com o nome vazio
...
void exibir(Livro *lista) {
    ...
    while (atual != NULL) {     //Percorre toda a lista
        printf("ID: %d \nTítulo: %s \nAutor: %s \nAno: %d \nGênero: %s \nEmprestado: %s \nPara: %s\n\n", atual->id, atual->titulo, atual->autor, atual->ano, atual->genero, atual->emprestado ? "Sim" : "Não", atual->emprestado ? atual->emprestadoPara : "");
...
Livro * buscarPorTitulo(const char *titulo) {
    ...
    {
        if (strcmp(atual->titulo, titulo) == 0 && !atual->emprestado)   //Compara o título e checa se não está emprestado
        {
...
Livro *buscarPorAutor(const char *autor) {
    ...
    while (atual != NULL) {
        if (strcmp(atual->autor, autor) == 0 && !atual->emprestado)   //Compara o autor e checa se não está emprestado
        {
            return atual;
        }
        atual = atual->proximo;
...
Livro * buscarPorAno(int ano) {
    ...
    while (atual != NULL)   //Percorre a lista de colisões
    {
        if (atual->ano == ano && !atual->emprestado)     //Compara o ano e checa se não está emprestado
        {
            return atual;   //Retorna o livro encontrado
        }
        atual = atual->proximo;
...
void registrarEmprestimo(const char *titulo, const char *nomePessoa){
    ...
    if (livro != NULL)
    {
        if (livro->emprestado)
        {
            printf("O livro '%s' já se encontra emprestado para %s.\n", titulo, livro->emprestadoPara);
        } else
        {
            livro->emprestado = 1;
            strcpy(livro->emprestadoPara, nomePessoa);
...
void registrarDevolucao(const char *titulo) {
    ...
    Livro *atual = hashTitulo[index];
    while (atual != NULL) {
        if (strcmp(atual->titulo, titulo) == 0 && atual->emprestado) {
            atual->emprestado = 0;
    ...
    printf("Livro com título '%s' não encontrado ou não está emprestado.\n", titulo);
}
    ...
    do
    {
        ...
        case 3:
            ...
            case 6:
                ...
                printf("Digite o título do livro a emprestar: ");
                getchar();
                fgets(titulo, sizeof(titulo), stdin);
                titulo[strcspn(titulo, "\n")] = '\0';
                ...
                fgets(autor, sizeof(autor), stdin);
                autor[strcspn(autor, "\n")] = '\0';
                registrarEmprestimo(titulo, autor);
                break;
             case 7:
            printf("Digite o título do livro a devolver: ");
            getchar();
            fgets(titulo, sizeof(titulo), stdin);
            titulo[strcspn(titulo, "\n")] = '\0';
